...
func (s *Store) CreateToken(ctx context.Context, token *domain.APIToken) error {
	...
	_, err = s.db.ExecContext(ctx, s.q(`
		INSERT INTO api_tokens (id, workspace_id, name, token_hash, scopes, expires_at, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?)`),
...
func (s *Store) GetTokenByHash(ctx context.Context, hash []byte) (*domain.APIToken, error) {
	row := s.db.QueryRowContext(ctx, s.q(`
		SELECT id, workspace_id, name, token_hash, scopes, expires_at, revoked_at, created_at
...
func scanToken(row *sql.Row, driver Driver) (*domain.APIToken, error) {
	...
	if err := row.Scan(&id, &workspaceID, &name, &tokenHash, &scopes, &expiresAt, &revokedAt, &createdAt); err != nil {
