...
> **Status: Completed** — branch `feat/secrets-s2`  
...
- [x] Parse base64 32-byte key; Encrypt/Decrypt with AES-GCM; `v1:` prefix; IsCiphertext
- [x] Unit tests round-trip + bad key + wrong ciphertext
- [x] Commit: `feat(secrets): AES-GCM box for config values`
...
- [x] `Store.WithSecrets(*secrets.Box)`; nil box = no encryption (plain-only OK)
- [x] Merge: encrypt when sensitivity=secret (fail without key)
- [x] List/resolve: decrypt ciphertext secrets; legacy plaintext secrets pass through
- [x] CreateRelease: seal secret keys in config_resolved JSON; scanRelease: open
- [x] Tests: encrypt round-trip, refuse write without key, deploy path plaintext in memory
- [x] Commit: `feat(store): encrypt secret config at rest`
...
- [x] Load `LAUNCHPAD_SECRETS_KEY` in API + worker; same key both processes
- [x] e2e-stub exports fixed test key to API + worker
- [x] Update sensitivity tests with test box
- [x] Commit: `feat(cmd): wire LAUNCHPAD_SECRETS_KEY into api and worker`
...
- [x] DOMAIN: S2 storage row; residual risk note for unencrypted legacy rows until rewrite
- [x] QUEUE: secrets-s1 → shipped; secrets-s2 → pr-open; env-clone still blocked until S2 preferred ships (or note min S1)
- [x] README env table
- [x] Commit: `docs: secrets S2 encryption and queue`
