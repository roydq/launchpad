...
Mutating **production** / **prod** requires `--yes`:

```bash
launchpad env create staging --target stub
launchpad env create production --target stub
launchpad env use staging
launchpad deploy --image my-api:v1 --wait
launchpad promote --from staging --to production --wait --yes
# or: launchpad promote --from staging --to production --release 1 -m "ship" --wait --yes
```

API errors print recovery **hints** when the control plane returns problem+json `code` / `hints`.
